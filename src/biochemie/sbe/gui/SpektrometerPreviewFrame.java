...
//        xyplot.setDomainAxis(axis);
     ...
     * @param showions 
     */
    private void addDataset(double[] m, String[] dataline, String id, String snp, XYSeriesCollection collection, boolean showions) {
        ...
        for (int i = 0; i < snp.length(); i++) {
            ...
            masse.add(m[idx],LEN*5/4);
            l.add(name+": "+df.format(m[idx])+"D");
            if(showions){
                masse.add(m[idx]+22,LEN*5/4f*1/7f);
                l.add(name+", Na+ Peak: "+df.format(m[idx]+22)+"D");
                masse.add(m[idx]+38,LEN*5/4f*1/10f);
                l.add(name+", K+ Peak: "+df.format(m[idx]+38)+"D");
            }
        }
